...
- **Stream only while popover is open** — `watchTabs` is wired through a runtime atom and only subscribes when the backlink popover is open, avoiding unnecessary message traffic.
