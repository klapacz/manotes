...
export class GraphSyncDurableObject extends DurableObject<Env> {
  ...
  private executeWebSocketMessage(message: Messages.ClientMessage) {
    ...
    return Boolean.match(plan.runSerialized, {
      ...
      onTrue: () => this.ctx.blockConcurrencyWhile(() => this.runtime.runPromiseExit(plan.effect)),
    });
  }

  private respondToWebSocketMessage(ws: WebSocket, result: Protocol.ResponsePlan): void {
    for (const response of result.reply) ws.send(Codec.encodeServerMessageUnsafe(response));
  ...
  private handleWebSocketFailure<E>(
    ...
    this.runtime.runFork(Effect.logError("Graph sync websocket failure", Cause.pretty(cause)));
  ...
  webSocketError(_ws: WebSocket, error: unknown): void {
    this.runtime.runFork(Effect.logWarning("Graph sync websocket error", error));
  }

  webSocketClose(_ws: WebSocket, code: number, reason: string): void {
    this.runtime.runFork(Effect.logWarning("Graph sync websocket closed", { code, reason }));
  }
}
