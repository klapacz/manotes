 ...
 * detects the change.
 */
const [scrollToDateRequest, setScrollToDateRequest] = createSignal<Record<string, never> | null>(
  null,
);
...
export function suggestDailyNoteIds(
  ...
  if (exact) {
    return [{ id: exact.toString(), title: formatDailyNoteTitle(exact.toString()) }];
  }
