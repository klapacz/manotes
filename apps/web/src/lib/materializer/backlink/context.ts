...
import { appNodeFromJSON, findParentNode, transformNode } from "../../prosemirror/utils";
 ...
 */
function extractBacklinkContext(doc: Node, backlink: BacklinkInfo): ContextInfo {
  ...
  const heading = findParentNode((node) => node.type.name === "heading", $backlink);
 ...
 */
function extractBacklinkContextForList(doc: Node, $backlink: ResolvedPos): ContextInfo | undefined {
  const immediateParentList = findParentNode((node) => node.type.name === "list", $backlink);
  ...
  return {
    node: higherLevelParentList.node.type.create(
      ...
      leadingParagraph ? [leadingParagraph, immediateParentList.node] : [immediateParentList.node],
    ),
...
function uncollapseLists(node: Node): Node {
  return transformNode(node, (currentNode) => {
    ...
    return currentNode.type.create({ ...currentNode.attrs, collapsed: false }, currentNode.content);
  });
}
