...
export class Service extends ServiceMap.Service<Service>()("GraphSyncEncryption.Service", {
  make: Effect.gen(function* () {
    ...
    const eventEncryptionKey = yield* Effect.tryPromise({
      try: () => crypto.subtle.importKey("raw", graphKey, "AES-GCM", false, ["encrypt", "decrypt"]),
      catch: (cause) => new EventEncryptionError({ cause }),
    });

    const deriveNoteStreamRef = Effect.fn("GraphSyncEncryptionService.deriveNoteStreamRef")(
      function* (noteId: string) {
        const signature = yield* Effect.tryPromise({
          try: () => crypto.subtle.sign("HMAC", streamRefKey, encodeStreamRefInput(noteId)),
          catch: (cause) => new StreamRefDerivationError({ cause }),
        });
    ...
    const encryptEventBody = Effect.fn("GraphSyncEncryptionService.encryptEventBody")(function* ({
      id,
      streamRef,
      createdAt,
      noteId,
    ...
    const decryptEventBody = Effect.fn("GraphSyncEncryptionService.decryptEventBody")(function* ({
      id,
      streamRef,
      createdAt,
      envelope,
