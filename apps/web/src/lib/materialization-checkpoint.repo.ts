import { DateTime, Effect, Layer, Option, ServiceMap, Stream, Array } from "effect";
    ...
    make: Effect.gen(function* () {
      ...
      const getOrInit = Effect.fn("MaterializationCheckpointRepo.getOrInit")(function* () {
        ...
        return yield* Option.match(inserted, {
          onNone: () =>
            Effect.die(new Error("Failed to initialize materialization checkpoint row.")),
      ...
      const waitUntilAtLeast = Effect.fn("MaterializationCheckpointRepo.waitUntilAtLeast")(
        function* (targetLocalSeq: number) {
          ...
          return yield* Option.match(reached, {
            onNone: () =>
              Effect.die(
                new Error("Checkpoint stream ended before target event was materialized."),
              ),
            onSome: () => Effect.void,
          });
        },
