...
export class Service extends Effect.Service<Service>()("MaterializationCheckpointRepo.Service", {
  ...
  effect: Effect.gen(function* () {
    ...
    const getOrInit = Effect.fn("MaterializationCheckpointRepo.getOrInit")(function* () {
      ...
      return yield* Option.match(inserted, {
        onNone: () => Effect.dieMessage("Failed to initialize materialization checkpoint row."),
    ...
    const waitUntilAtLeast = Effect.fn("MaterializationCheckpointRepo.waitUntilAtLeast")(function* (
      ...
      return yield* Option.match(reached, {
        onNone: () =>
          Effect.dieMessage("Checkpoint stream ended before target event was materialized."),
        onSome: () => Effect.void,
      });
    });
