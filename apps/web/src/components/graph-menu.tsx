...
export const GraphMenu = (props: { graph: LocalRegistry.Schema.Record }) => {
  const [exportBackupResult, exportBackup] = RtAtom.use(ExportBackup, { mode: "promise" });

  async function handleExportBackup() {
    try {
      await exportBackup(props.graph.displayName);
        ...
        <DropdownMenu placement="top-start">
          <DropdownMenuTrigger
            as={(triggerProps: DropdownMenuTriggerProps<HTMLButtonElement>) => (
              ...
              >
                <GraphMenuIdentity graph={props.graph} />
          ...
          <DropdownMenuPortal>
            <DropdownMenuContent class="w-(--kb-popper-anchor-width) min-w-56 rounded-lg">
              ...
              <DropdownMenuGroup>
                <DropdownMenuItem
                  onSelect={() => void handleExportBackup()}
