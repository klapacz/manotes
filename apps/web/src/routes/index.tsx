...
function RouteComponent() {
  ...
  const cloudGraphIdsOnDevice = createMemo(
    () => new Set(graphs.map((graph) => graph.graphId).filter((graphId) => graphId !== null)),
  );

  const cloudGraphsNotOnDevice = createMemo(() =>
    (cloudGraphsQuery.data ?? []).filter((graph) => !cloudGraphIdsOnDevice().has(graph.graphId)),
  );
    ...
    async mutationFn(graph: RemoteGraphRegistry.Graph) {
      ...
      throw new Error(
        Boolean.match(isDisplayNameTakenError, {
          onTrue: constant("A graph with that name already exists on this device."),
