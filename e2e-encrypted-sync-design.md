...
### Long Offline Periods - Pending Updates Management

**Problem:** Client accumulates unbounded pending updates when offline for extended periods

**Solution:** Pending updates compaction (local merging)

Even when never syncing with the server, clients can merge their pending Yjs updates locally using `Y.mergeUpdates()`:

```typescript
// Before compaction: 1000+ small updates in pending array
pending.updates = [update1, update2, ..., update1000];

// After pending updates compaction: single merged Yjs update
...
- This is separate from server log compaction - it only affects local pending changes, not synced updates

**Note:** This is about managing **pending** (not-yet-pushed) updates. Compacting server_log updates while offline provides no benefit since the compact cannot be pushed to trigger server GC.
