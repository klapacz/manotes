...
export const run = (options: OpfsWorkerConfig): Effect.Effect<void, SqlError> =>
  Effect.gen(function* () {
    const factory = yield* Effect.promise(() =>
      SQLiteESMFactory({ locateFile: () => wasmUrl }),
    );
    const sqlite3 = WaSqlite.Factory(factory);
    const vfs = yield* Effect.promise(() =>
      OPFSCoopSyncVFS.create("opfs", factory),
    );
      ...
      const onMessage = async (event: any) => {
        ...
        try {
          switch (message[0]) {
            ...
            case "update_hook": {
              messageId = -1;
              updateBroadcast = new BroadcastChannel(
                `update_hook:${options.dbName}`,
              );
            ...
            default: {
              ...
              for await (const stmt of sqlite3.statements(db, sql)) {
                await sqlite3.bind_collection(stmt, params as any);
