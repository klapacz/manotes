  ...
  Effect.gen(function* () {
    ...
    const dedicatedWorkerRef = yield* Scope.extend(
      ...
      ScopedRef.fromAcquire(
        Effect.succeed<Option.Option<DedicatedClient>>(Option.none()),
      ),
      parentScope,
    );
        ...
        function* (payload) {
          ...
          if (Option.isSome(existingConnection)) {
            yield* Effect.logInfo(
              "Replacing existing connection (leader failover)",
            );
          }
            ...
            Effect.catchAllCause((cause) =>
              Effect.gen(function* () {
                ...
                yield* Effect.logError(
                  "Failed to create dedicated worker client",
                  cause,
                );
                return false;
              }),
