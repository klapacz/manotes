    ...
    Effect.async<Option.Option<LeadershipLock>>((resume) => {
      ...
      navigator.locks
        .request(lockName, { ifAvailable: true }, async (lock) => {
          ...
          resume(
            Effect.succeed(
              Option.some(new LeadershipLock({ release: lockHold.release })),
            ),
          );
        ...
        .request(lockName, { signal }, async (lock) => {
          if (!lock) {
            resume(
              Effect.die(
                new Error(
                  "Web Locks returned null lock for blocking leadership request.",
                ),
              ),
            );
            return;
          ...
          const lockHold = makeLockHold();
          resume(
            Effect.succeed(new LeadershipLock({ release: lockHold.release })),
          );
          return lockHold.hold;
        })
        .catch((error) => {
